        ...
        if (blockAccessor.getBlockEntity() instanceof IMachineBlockEntity blockEntity) {
            if (blockEntity.getMetaMachine() instanceof MEPatternBufferProxyPartMachine) {
                ...
                if (!serverData.getBoolean("bound")) {
                    iTooltip.add(Component.translatable("gt6.top.buffer_not_bound").withStyle(ChatFormatting.RED));
                    return;
                }

                int[] pos = serverData.getIntArray("pos");
                iTooltip.add(Component.translatable("gt6.top.buffer_bound_pos", pos[0], pos[1], pos[2])
