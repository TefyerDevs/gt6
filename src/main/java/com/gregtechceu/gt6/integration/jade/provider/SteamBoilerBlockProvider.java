...
public class SteamBoilerBlockProvider extends BlockInfoProvider<SteamBoilerMachine> {
    ...
    protected void addTooltip(CompoundTag capData, ITooltip tooltip, Player player, BlockAccessor block,
        ...
        if (isBurning && temperature < maxTemperature) {
            // Heating up
            root = Component.translatable("gt6.machine.boiler.info.heating.up");
        } else if (!isBurning && temperature > 0) {
            // Cooling down
            root = Component.translatable("gt6.machine.boiler.info.cooling.down");
        } else {
        ...
        if (makingSteam) {
            ...
            extra = Component.translatable("gt6.machine.boiler.info.production.data",
