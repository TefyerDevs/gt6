        ...
        if (compoundTag.contains("hasProblems", Tag.TAG_BYTE)) {
            if (compoundTag.getBoolean("hasProblems")) {
                if (blockAccessor.showDetails()) {
                    ...
                    iTooltip.add(Component.translatable("gt6.top.maintenance_broken").withStyle(ChatFormatting.RED));
                }
            } else {
                iTooltip.add(Component.translatable("gt6.top.maintenance_fixed").withStyle(ChatFormatting.GREEN));
            }
        }
    }
}
