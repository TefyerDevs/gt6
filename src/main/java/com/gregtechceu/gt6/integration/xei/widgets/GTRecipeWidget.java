package com.gregtechceu.gt6.integration.xei.widgets;

import com.gregtechceu.gt6.Gregtech;
import com.gregtechceu.gt6.api.GTValues;
import com.gregtechceu.gt6.api.capability.recipe.CWURecipeCapability;
...
import com.gregtechceu.gt6.api.gui.WidgetUtils;
import com.gregtechceu.gt6.api.gui.widget.PredicatedButtonWidget;
...
import com.gregtechceu.gt6.common.recipe.condition.DimensionCondition;
...
public class GTRecipeWidget extends WidgetGroup {
    ...
    private void initializeRecipeTextWidget() {
        ...
        if (EUt.voltage() > 0) {
            textsY += 10;
            Component text = Component.translatable(EUt.isInput() ? "gt6.recipe.eu" : "gt6.recipe.eu_inverted",
            ...
            recipeVoltageText.setHoverTooltips(
                    Component.translatable("gt6.recipe.eu.total", FormattingUtil.formatNumbers(EUt.getTotalEU()))
                            .withStyle(ChatFormatting.UNDERLINE));
        ...
        if (EUt.isInput()) {
            ...
            addWidget(new ButtonWidget(voltageTextWidget.getPositionX(), voltageTextWidget.getPositionY(),
                    ...
                    .setHoverTooltips(LangHandler.getMultiLang("gt6.oc.tooltip", GTValues.VNF[minTier])
    ...
    private static List<Component> getRecipeParaText(GTRecipe recipe, int duration,
        ...
        if (!recipe.data.getBoolean("hide_duration")) {
            texts.add(Component.translatable("gt6.recipe.duration", FormattingUtil.formatNumbers(duration / 20f)));
        }
        if (eu.voltage() > 0) {
            ...
            if (recipe.data.getBoolean("duration_is_total_cwu") &&
                ...
                texts.add(Component.translatable("gt6.recipe.max_eu",
                        FormattingUtil.formatNumbers(euTotal / minimumCWUt)));
            } else {
                texts.add(Component.translatable("gt6.recipe.total", FormattingUtil.formatNumbers(euTotal)));
            }
        }
    ...
    private void setRecipeOverclockWidget(OverclockingLogic logic) {
        ...
        if (recipeVoltageText != null) {
            recipeVoltageText.setComponent(Component.translatable("gt6.recipe.eu",
                    FormattingUtil.formatNumber2Places(minAmperage), GTValues.VN[minVoltageTier])
                    .withStyle(ChatFormatting.UNDERLINE));
            recipeVoltageText.setHoverTooltips(
                    Component.translatable("gt6.recipe.eu.total", FormattingUtil.formatNumbers(inputEUt.getTotalEU()))
                            .withStyle(ChatFormatting.UNDERLINE));
        }
    ...
    public static void setConsumedChance(Content content, ChanceLogic logic, List<Component> tooltips, int recipeTier,
        ...
        if (content.chance < ChanceLogic.getMaxChancedValue()) {
            ...
            if (boostedChance == 0) {
                tooltips.add(Component.translatable("gt6.gui.content.chance_nc"));
            } else {
                ...
                if (content.tierChanceBoost != 0) {
                    ...
                    if (logic != ChanceLogic.NONE && logic != ChanceLogic.OR) {
                        tooltips.add(Component.translatable("gt6.gui.content.chance_base_logic",
                                FormattingUtil.formatNumber2Places(baseChanceFloat), logic.getTranslation())
                                .withStyle(ChatFormatting.YELLOW));
                    } else {
                        tooltips.add(
                                FormattingUtil.formatPercentage2Places("gt6.gui.content.chance_base",
                                        baseChanceFloat));
                    }

                    String key = "gt6.gui.content.chance_tier_boost_" +
                    ...
                    if (logic != ChanceLogic.NONE && logic != ChanceLogic.OR) {
                        tooltips.add(Component.translatable("gt6.gui.content.chance_boosted_logic",
                                FormattingUtil.formatNumber2Places(boostedChanceFloat), logic.getTranslation())
                                .withStyle(ChatFormatting.YELLOW));
                    } else {
                        tooltips.add(
                                FormattingUtil.formatPercentage2Places("gt6.gui.content.chance_boosted",
                                        boostedChanceFloat));
                    }
                } else {
                    if (logic != ChanceLogic.NONE && logic != ChanceLogic.OR) {
                        tooltips.add(Component.translatable("gt6.gui.content.chance_no_boost_logic",
                                FormattingUtil.formatNumber2Places(baseChanceFloat), logic.getTranslation())
                                .withStyle(ChatFormatting.YELLOW));
                    } else {
                        tooltips.add(
                                FormattingUtil.formatPercentage2Places("gt6.gui.content.chance_no_boost",
                                        baseChanceFloat));
                    }
                }
            }
