...
public class GTOreVeinWidget extends WidgetGroup {
    ...
    private void setupBaseGui(GTOreDefinition oreDefinition) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            oreSlot.setOnAddedTooltips((stack, tooltips) -> tooltips.add(Component
                    .nullToEmpty(
                            LocalizationUtils.format("gt6.jei.ore_vein_diagram.chance", chances.get(finalIndex)))));
    ...
    private void setupBaseGui(BedrockOreDefinition bedrockOreDefinition) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            oreSlot.setOnAddedTooltips((stack, tooltips) -> tooltips.add(Component
                    .nullToEmpty(
                            LocalizationUtils.format("gt6.jei.ore_vein_diagram.chance",
    ...
    private void setupText(GTOreDefinition ignored) {
        addWidget(new ImageWidget(5, 0, width - 10, 16,
                new TextTexture("gt6.jei.ore_vein." + name).setType(TextTexture.TextType.LEFT_ROLL)
                        .setWidth(width - 10)));
        addWidget(new LabelWidget(5, 40,
                LocalizationUtils.format("gt6.jei.ore_vein_diagram.spawn_range")));
        addWidget(new LabelWidget(5, 50, range));

        addWidget(new LabelWidget(5, 60,
                LocalizationUtils.format("gt6.jei.ore_vein_diagram.weight", weight)));
        addWidget(new LabelWidget(5, 70,
                LocalizationUtils.format("gt6.jei.ore_vein_diagram.dimensions")));
        setupDimensionMarker(80);
    }

    private void setupText(BedrockFluidDefinition ignored) {
        addWidget(new ImageWidget(5, 0, width - 10, 16,
                new TextTexture("gt6.jei.bedrock_fluid." + name).setType(TextTexture.TextType.LEFT_ROLL)
                        .setWidth(width - 10)));
        addWidget(new LabelWidget(5, 40,
                LocalizationUtils.format("gt6.jei.ore_vein_diagram.weight", weight)));
    ...
    private void setupText(BedrockOreDefinition ignored) {
        addWidget(new ImageWidget(5, 0, width - 10, 16,
                new TextTexture("gt6.jei.bedrock_ore." + name).setType(TextTexture.TextType.LEFT_ROLL)
                        .setWidth(width - 10)));
        addWidget(new LabelWidget(5, 40,
                LocalizationUtils.format("gt6.jei.ore_vein_diagram.weight", weight)));
        addWidget(new LabelWidget(5, 50,
