...
import com.gregtechceu.gt6.api.gui.fancy.FancyMachineUIWidget;
import com.gregtechceu.gt6.api.gui.fancy.IFancyUIProvider;
import com.gregtechceu.gt6.api.gui.fancy.TooltipsPanel;
import com.gregtechceu.gt6.api.machine.ConditionalSubscriptionHandler;
...
public class PowerSubstationMachine extends WorkableMultiblockMachine
    ...
    public void addDisplayText(List<Component> textList) {
        ...
        if (isFormed()) {
            ...
            if (energyBank != null) {
                ...
                textList.add(Component.translatable("gt6.multiblock.power_substation.stored",
                        storedComponent.setStyle(STYLE_GOLD)));

                var capacityComponent = Component.literal(FormattingUtil.formatNumbers(energyCapacity));
                textList.add(Component.translatable("gt6.multiblock.power_substation.capacity",
                ...
                textList.add(Component.translatable("gt6.multiblock.power_substation.passive_drain",
                ...
                textList.add(Component
                        .translatable("gt6.multiblock.power_substation.average_in",
                                avgInComponent.setStyle(STYLE_GREEN))
                        .withStyle(Style.EMPTY.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                Component.translatable("gt6.multiblock.power_substation.average_in_hover")))));

                var avgOutComponent = Component.literal(FormattingUtil.formatNumbers(Math.abs(outputPerSec / 20)));
                textList.add(Component
                        .translatable("gt6.multiblock.power_substation.average_out",
                                avgOutComponent.setStyle(STYLE_RED))
                        .withStyle(Style.EMPTY.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                Component.translatable("gt6.multiblock.power_substation.average_out_hover")))));
                ...
                if (inputPerSec > outputPerSec) {
                    ...
                    textList.add(Component.translatable("gt6.multiblock.power_substation.time_to_fill",
                    ...
                    textList.add(Component.translatable("gt6.multiblock.power_substation.time_to_drain",
    ...
    private static MutableComponent getTimeToFillDrainText(BigInteger timeToFillSeconds) {
        ...
        if (duration.getSeconds() <= 180) {
            ...
            key = "gt6.multiblock.power_substation.time_seconds";
        } else if (duration.toMinutes() <= 180) {
            fillTime = duration.toMinutes();
            key = "gt6.multiblock.power_substation.time_minutes";
        } else if (duration.toHours() <= 72) {
            fillTime = duration.toHours();
            key = "gt6.multiblock.power_substation.time_hours";
        } else if (duration.toDays() <= 730) { // 2 years
            fillTime = duration.toDays();
            key = "gt6.multiblock.power_substation.time_days";
        } else if (duration.toDays() / 365 < 1_000_000) {
            fillTime = duration.toDays() / 365;
            key = "gt6.multiblock.power_substation.time_years";
        } else {
            return Component.translatable("gt6.multiblock.power_substation.time_forever");
        }
