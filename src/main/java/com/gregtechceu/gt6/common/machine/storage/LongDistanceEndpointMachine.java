package com.gregtechceu.gt6.common.machine.storage;
...
import com.gregtechceu.gt6.api.pipenet.longdistance.ILDEndpoint;
import com.gregtechceu.gt6.api.pipenet.longdistance.LongDistanceNetwork;
...
public abstract class LongDistanceEndpointMachine extends MetaMachine implements ILDEndpoint, IDataInfoProvider {
    ...
    public List<Component> getDataInfo(PortableScannerBehavior.DisplayMode mode) {
        ...
        if (mode == PortableScannerBehavior.DisplayMode.SHOW_ALL ||
            ...
            LongDistanceNetwork network = LongDistanceNetwork.get(getLevel(), getPos());
            if (network == null) {
                textComponents.add(Component.translatable("block.gt6.long_distance_item_pipeline_no_network"));
            } else {
                textComponents.add(Component.translatable("block.gt6.long_distance_item_pipeline_network_header"));
                textComponents.add(Component.translatable("block.gt6.long_distance_item_pipeline_pipe_count",
                ...
                textComponents.add(Component.translatable("block.gt6.long_distance_item_pipeline_input_pos",
                        Component.literal(in == null ? "none" : in.getPos().toString())));
                textComponents.add(Component.translatable("block.gt6.long_distance_item_pipeline_output_pos",
                        Component.literal(out == null ? "none" : out.getPos().toString())));
            }
            if (isInput()) {
                textComponents.add(Component.translatable("block.gt6.long_distance_item_pipeline_input_endpoint"));
            }
            if (isOutput()) {
                textComponents.add(Component.translatable("block.gt6.long_distance_item_pipeline_output_endpoint"));
            }
        }
