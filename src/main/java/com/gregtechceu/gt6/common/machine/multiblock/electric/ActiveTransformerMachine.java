...
import com.gregtechceu.gt6.api.machine.feature.IExplosionMachine;
...
import static com.gregtechceu.gt6.api.pattern.Predicates.abilities;
        ...
        if (isFormed()) {
            if (!isWorkingEnabled()) {
                ...
                textList.add(Component
                        .translatable("gt6.multiblock.active_transformer.max_input",
                                FormattingUtil.formatNumbers(
                                        Math.abs(powerInput.getInputVoltage() * powerInput.getInputAmperage()))));
                textList.add(Component
                        .translatable("gt6.multiblock.active_transformer.max_output",
                                FormattingUtil.formatNumbers(
                                        Math.abs(powerOutput.getOutputVoltage() * powerOutput.getOutputAmperage()))));
                textList.add(Component
                        .translatable("gt6.multiblock.active_transformer.average_in",
                                FormattingUtil.formatNumbers(Math.abs(powerInput.getInputPerSec() / 20))));
                textList.add(Component
                        .translatable("gt6.multiblock.active_transformer.average_out",
                ...
                if (!ConfigHolder.INSTANCE.machines.harmlessActiveTransformers) {
                    textList.add(Component
                            .translatable("gt6.multiblock.active_transformer.danger_enabled"));
                }
            } else {
