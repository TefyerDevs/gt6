...
public record AntidoteBehavior(Set<MedicalCondition> types, int removePercent)
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltipComponents,
        ...
        if (GTUtil.isShiftDown()) {
            tooltipComponents.add(Component.translatable("gt6.medical_condition.antidote.description_shift"));
            for (var type : types) {
                tooltipComponents.add(Component.translatable("gt6.medical_condition." + type.name));
            }
            if (removePercent == -1) {
                tooltipComponents
                        .add(Component.translatable("gt6.medical_condition.antidote.description.effect_removed.all"));
            } else {
                tooltipComponents
                        .add(Component.translatable("gt6.medical_condition.antidote.description.effect_removed",
                                removePercent));
            }
            return;
        }
        tooltipComponents.add(Component.translatable("gt6.medical_condition.antidote.description"));
    }
}
