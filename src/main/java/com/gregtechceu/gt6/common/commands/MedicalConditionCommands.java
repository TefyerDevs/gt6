...
import com.gregtechceu.gt6.common.commands.arguments.MedicalConditionArgument;
    ...
    private static int queryMedicalConditions(ServerPlayer target) throws CommandSyntaxException {
        ...
        if (count == 0) {
            target.sendSystemMessage(
                    Component.translatable("command.gt6.medical_condition.get.empty", target.getName()));
        } else {
            target.sendSystemMessage(
                    Component.translatable("command.gt6.medical_condition.get", target.getName()));
        }
        for (var entry : tracker.getMedicalConditions().object2FloatEntrySet()) {
            String langKey = "command.gt6.medical_condition.get.element";
            if (entry.getKey().maxProgression * 2 <= entry.getFloatValue() &&
                    entry.getKey().canBePermanent) {
                langKey = "command.gt6.medical_condition.get.element.permanent";
            }
            float time = entry.getFloatValue();
            target.sendSystemMessage(
                    Component.translatable(langKey,
                            Component.translatable("gt6.medical_condition." + entry.getKey().name),
