...
import com.gregtechceu.gt6.api.fluids.FluidConstants;
import com.gregtechceu.gt6.api.fluids.FluidState;
import com.gregtechceu.gt6.api.fluids.GTFluid;
...
public class TooltipsHandler {
    ...
    public static void appendFluidTooltips(FluidStack fluidStack, Consumer<Component> tooltips, TooltipFlag flag) {
        ...
        if (!material.isNull()) {
            ...
            if (material.hasProperty(PropertyKey.INGOT)) {
                if (GTUtil.isShiftDown() && amount >= GTValues.L) {
                    ...
                    tooltips.accept(Component.translatable("gt6.gui.fluid_amount").withStyle(ChatFormatting.GRAY)
        ...
        if (fluid instanceof GTFluid attributedFluid) {
            ...
            switch (state) {
                case LIQUID -> tooltips.accept(Component.translatable("gt6.fluid.state_liquid"));
                case GAS -> tooltips.accept(Component.translatable("gt6.fluid.state_gas"));
                case PLASMA -> tooltips.accept(Component.translatable("gt6.fluid.state_plasma"));
            }
            attributedFluid.getAttributes().forEach(a -> a.appendFluidTooltips(tooltips));
        } else {
            String key = "gt6.fluid.state_" + (fluidType.isLighterThanAir() ? "gas" : "liquid");
            tooltips.accept(Component.translatable(key));
        }

        tooltips.accept(Component.translatable("gt6.fluid.temperature", fluidType.getTemperature()));
        if (fluidType.getTemperature() < FluidConstants.CRYOGENIC_FLUID_THRESHOLD) {
            tooltips.accept(Component.translatable("gt6.fluid.temperature.cryogenic"));
        }
    }
}
