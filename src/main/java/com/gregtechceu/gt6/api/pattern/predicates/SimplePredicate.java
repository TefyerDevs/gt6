package com.gregtechceu.gt6.api.pattern.predicates;
...
import com.gregtechceu.gt6.api.pattern.error.SinglePredicateError;
...
public class SimplePredicate {
    ...
    public List<Component> getToolTips(TraceabilityPredicate predicates) {
        ...
        if (minCount == maxCount && maxCount != -1) {
            result.add(Component.translatable("gt6.multiblock.pattern.error.limited_exact", minCount));
        } else if (minCount != maxCount && minCount != -1 && maxCount != -1) {
            result.add(Component.translatable("gt6.multiblock.pattern.error.limited_within", minCount, maxCount));
        } else {
            if (minCount != -1) {
                result.add(LangHandler.getFromMultiLang("gt6.multiblock.pattern.error.limited", 1, minCount));
            }
            if (maxCount != -1) {
                result.add(LangHandler.getFromMultiLang("gt6.multiblock.pattern.error.limited", 0, maxCount));
            }
        }
        if (predicates == null) return result;
        if (predicates.isSingle()) {
            result.add(Component.translatable("gt6.multiblock.pattern.single"));
        }
        if (predicates.hasAir()) {
            result.add(Component.translatable("gt6.multiblock.pattern.replaceable_air"));
        }
        return result;
    }
