...
import com.gregtechceu.gt6.api.misc.forge.SimpleThermalFluidHandlerItemStack;
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltipComponents,
        ...
        if (stack.hasTag()) {
            FluidUtil.getFluidContained(stack).ifPresent(tank -> {
                tooltipComponents
                        .add(Component.translatable("gt6.universal.tooltip.fluid_stored", tank.getDisplayName(),
                                tank.getAmount()));
                TooltipsHandler.appendFluidTooltips(tank, tooltipComponents::add, null);
            });
        } else {
            tooltipComponents.add(Component.translatable("gt6.universal.tooltip.fluid_storage_capacity",
                    FormattingUtil.formatNumbers(capacity)));
        }
        if (GTUtil.isShiftDown()) {
            tooltipComponents.add(Component.translatable("gt6.fluid_pipe.max_temperature",
                    FormattingUtil.formatNumbers(maxFluidTemperature)));
            if (gasProof) tooltipComponents.add(Component.translatable("gt6.fluid_pipe.gas_proof"));
            else tooltipComponents.add(Component.translatable("gt6.fluid_pipe.not_gas_proof"));
            if (plasmaProof) tooltipComponents.add(Component.translatable("gt6.fluid_pipe.plasma_proof"));
            if (cryoProof) tooltipComponents.add(Component.translatable("gt6.fluid_pipe.cryo_proof"));
            if (acidProof) tooltipComponents.add(Component.translatable("gt6.fluid_pipe.acid_proof"));
        } else if (gasProof || cryoProof || plasmaProof || acidProof) {
            tooltipComponents.add(Component.translatable("gt6.tooltip.fluid_pipe_hold_shift"));
        }
    }
}
